...
def statistical_feature_extraction(
    ...
    [phasic_gsr, p, tonic_gsr, l, d, e, obj] = cvxEDA(
        preprocessed_gsr, 1.0 / sample_rate
    )
...
def segmentwise(
    gsrdata, sample_rate, segment_width=120, segment_overlap=0, segment_min_size=5
):
    """processes passed gsrdata.
    Processes the passed gsr data. Returns measures{} dict containing results.
    ...
    slice_indices = make_windows(
        gsrdata, sample_rate, segment_width, segment_overlap, segment_min_size
    )
