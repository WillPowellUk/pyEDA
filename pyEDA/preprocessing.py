...
def rolling_mean(data, windowsize, sample_rate):
    ...
    t_shape = data_arr.shape[:-1] + (
        data_arr.shape[-1] - t_windowsize + 1,
        t_windowsize,
    )
    t_strides = data_arr.strides + (data_arr.strides[-1],)
    sep_win = np.lib.stride_tricks.as_strided(
        data_arr, shape=t_shape, strides=t_strides
    )
    rol_mean = np.mean(sep_win, axis=1)

    missing_vals = np.array(
        [avg_hr for i in range(0, int(abs(len(data_arr) - len(rol_mean)) / 2))]
    )
