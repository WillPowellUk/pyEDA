...
class AE(nn.Module):
    def __init__(self, **kwargs):
        ...
        self.linear1 = nn.Linear(
            in_features=kwargs["input_shape"], out_features=closest_pow2
        )
        ...
        self.linear2 = nn.Linear(
            in_features=(closest_pow2) * 2, out_features=kwargs["latent_size"]
        )

        # Decoder layers
        self.linear3 = nn.Linear(
            in_features=kwargs["latent_size"], out_features=(closest_pow2) * 2
        )
        ...
        self.linear4 = nn.Linear(
            in_features=closest_pow2, out_features=kwargs["input_shape"]
        )
    ...
    def forward(self, features):
        ...
        activation = torch.reshape(
            activation, (activation.shape[0], 1, activation.shape[1])
        )
        ...
        activation = torch.reshape(
            activation, (activation.shape[0], activation.shape[2])
        )
